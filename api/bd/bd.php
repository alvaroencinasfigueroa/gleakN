...
function conectar(){
    try{
        $GLOBALS['pdo']=new PDO("mysql:host=".$GLOBALS['host'].";dbname=".$GLOBALS['bd']."", $GLOBALS['user']);
        $GLOBALS['pdo']->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    }catch (PDOException $e){
        print "\nError!: No se pudo conectar a la bd: <br/>";
        print "\nError!: ".$e."<br/>";
        die();
    }
}
...
function metodoGet1($query){
    try{
        conectar();
        $aaaa=$GLOBALS['pdo']->prepare($query);
        $aaaa->setFetchMode(PDO::FETCH_ASSOC);
        $aaaa->execute();
        desconectar();
        return $aaaa;
    }catch(Exception $e){
